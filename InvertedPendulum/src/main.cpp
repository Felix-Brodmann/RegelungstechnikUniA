...
int main()
{
    try
    {
        ...
        control_engineering_uni_a::I2CDevice i2cDevice("/dev/i2c-1", 0x68);

        // Create an LSM6DS3 IMU sensor using the builder pattern
        std::unique_ptr<control_engineering_uni_a::LSM6DS3Builder> imu = 
            control_engineering_uni_a::LSM6DS3Builder::create()
            .withI2CDevice(i2cDevice)
        ...
        imuBuilder->addFilter(std::move(imuFilter));

        // Create a PWM device instance for the EV3 motor
        control_engineering_uni_a::PWMDevice pwmDevice(EV3_MOTOR_PWM_PIN, 1000); // 1 kHz PWM frequency

        // Create an EV3 motor using the builder pattern
        std::unique_ptr<control_engineering_uni_a::EV3MotorBuilder> motor = 
            control_engineering_uni_a::EV3MotorBuilder::create()
            .setPwmDevice(pwmDevice)
