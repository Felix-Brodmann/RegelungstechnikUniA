...
private:
    static constexpr uint8_t WHO_AM_I_REGISTER = 0x0F;
    static constexpr uint8_t ACCELEROMETER_CONFIG_REGISTER = 0x10;
    static constexpr uint8_t GYROSCOPE_CONFIG_REGISTER = 0x11;
    static constexpr uint8_t TEMPERATURE_DATA_REGISTER = 0x20;
    static constexpr uint8_t GYROSCOPE_DATA_REGISTER = 0x22;
    static constexpr uint8_t ACCELEROMETER_DATA_REGISTER = 0x28;
    ...
    Vector3D readAccelerometer()
    {
        ...
        switch(m_accelerometerUnit)
        {
            case AccelerometerUnit::IN_MG:
                accelerometerValue = Vector3D{x_d, y_d, z_d} * scale;
                break;

            case AccelerometerUnit::IN_G:
                accelerometerValue = Vector3D{x_d, y_d, z_d} * scale / 1000.0;
                break;

            case AccelerometerUnit::IN_M_PER_S2:
                accelerometerValue = Vector3D{x_d, y_d, z_d} * scale / 1000.0 * GRAVITY;
                break;
        }
    ...
    Vector3D readGyroscope()
    {
        ...
        switch(m_gyroscopeUnit)
        {
            case GyroscopeUnit::IN_MDEG_PER_S:
                gyroscopeValue = Vector3D{x_d, y_d, z_d} * scale - m_gyroscopeOffset;
                break;
            case GyroscopeUnit::IN_DEG_PER_S:
                gyroscopeValue = Vector3D{x_d, y_d, z_d} * scale / 1000.0 - m_gyroscopeOffset;
                break;
            case GyroscopeUnit::IN_RAD_PER_S:
                gyroscopeValue = Vector3D{x_d, y_d, z_d} * scale / 1000.0 * RAD_SCALE - m_gyroscopeOffset;
                break;
        }
...
public:
     ...
     * @param t_i2cDevice A unique pointer to the I2C device used to communicate with the sensor.
     ...
     */
    LSM6DS3(std::unique_ptr<I2CDevice> t_i2cDevice) : Sensor("LSM6DS3")
    {
