...
public:
     ...
     * @return Reference to the PRegulatorBuilder instance for method chaining.
     */
    PRegulatorBuilder& setOutputMin(const SensorData outputMin)
    {
     ...
     * @return Reference to the PRegulatorBuilder instance for method chaining.
     */
    PRegulatorBuilder& setOutputMax(const SensorData outputMax)
    {
    ...
    std::unique_ptr<PRegulator> build()
    {
        ...
        if (!m_outputMin.has_value()) {
            throw std::runtime_error("Output minimum must be set.");
        }
        if (!m_outputMax.has_value()) {
            throw std::runtime_error("Output maximum must be set.");
        }
