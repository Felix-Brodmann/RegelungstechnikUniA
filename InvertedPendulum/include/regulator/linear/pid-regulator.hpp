...
public:
    ...
    {
        // Check if the integral limits are of the same type and valid
        if (!std::holds_alternative<int>(t_integralMin) &&
            !std::holds_alternative<double>(t_integralMin) &&
            !std::holds_alternative<Vector2D>(t_integralMin) &&
            !std::holds_alternative<Vector3D>(t_integralMin))
        {
            throw std::invalid_argument("Integral minimum type is not supported, must be int, double, Vector2D or Vector3D");
        }
        if (!std::holds_alternative<int>(t_integralMax) &&
            !std::holds_alternative<double>(t_integralMax) &&
            !std::holds_alternative<Vector2D>(t_integralMax) &&
            !std::holds_alternative<Vector3D>(t_integralMax))
        {
            throw std::invalid_argument("Integral maximum type is not supported, must be int, double, Vector2D or Vector3D");
        }
        ...
        // Check if the minimum integral value is less than the maximum integral value
        if (t_integralMin > t_integralMax)
        {
            throw std::invalid_argument("Integral minimum cannot be greater than integral maximum");
        }
