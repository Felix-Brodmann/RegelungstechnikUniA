        ...
        while (isRegulating())
        {
            ...
            SensorData control = std::visit([this](auto&& val) -> SensorData
            {
                ...
                if constexpr (std::is_same_v<T, int> || std::is_same_v<T, double> || std::is_same_v<T, Vector2D> || std::is_same_v<T, Vector3D>)
                {
                    ...
                    std::cout << "Error: " << error << std::endl; // Debugging output
                    ...
                    std::cout << "Integral Error: " << integralError << std::endl; // Debugging output

                    // Calculate the derivative error
                    auto derivativeError = (error - std::get<T>(getLastError())) / dt;
                    std::cout << "Derivative Error: " << derivativeError << std::endl; // Debugging output
                    ...
                    return std::clamp<T>( error * getKp() + integralError * getKi() + derivativeError * getKd(),
...
public:
        ...
        : Regulator(t_regulatorName, std::move(t_sensor), std::move(t_actuator))
    {
        ...
        setInitilizationStatus("outputMin", false);
        setInitilizationStatus("outputMax", false);
    ...
    void setOutputLimits(SensorData t_outputMin, SensorData t_outputMax)
    {
        ...
        setInitilizationStatus("outputMin", true);
        setInitilizationStatus("outputMax", true);
    ...
    void startRegulation() override
    {
        ...
        std::cout << "Starting regulation for " << getRegulatorName() << std::endl;
    ...
    {
        std::cout << "Stopping regulation for " << getRegulatorName() << std::endl;
        setIsRegulating(false);

        std::cout << "Checking if the regulation thread is joinable..." << std::endl;
        // Join the regulation thread if it's running
        if (m_regulationThread.joinable())
        {
            std::cout << "Regulation thread is joinable, joining..." << std::endl;
            m_regulationThread.join();
            std::cout << "Regulation thread joined successfully." << std::endl;
