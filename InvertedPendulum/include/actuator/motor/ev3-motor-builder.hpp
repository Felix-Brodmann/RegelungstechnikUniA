...
{
private:
    std::unique_ptr<PwmDevice> m_pwmDevice; // PWM device for motor control
...
public:
    ...
    /**
     * @brief Default constructor for EV3MotorBuilder.
     * @details Initializes an empty EV3MotorBuilder instance.
     */
    EV3MotorBuilder(std::unique_ptr<PwmDevice> pwmDevice) : m_pwmDevice(std::move(pwmDevice)) {}

    static EV3MotorBuilder create(std::unique_ptr<PwmDevice> pwmDevice)
    {
        return EV3MotorBuilder(std::move(pwmDevice));
    }
    ...
    std::unique_ptr<EV3Motor> build()
    {
        ...
        motor->setPWMDevice(std::move(m_pwmDevice));
...
#endif // ACTOR_MOTOR_EV3_MOTOR_BUILDER_HPP